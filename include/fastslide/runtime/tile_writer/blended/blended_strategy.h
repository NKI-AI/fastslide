...
private:
  aifocore::Status WriteRGBTileBlended(const core::TileReadOp &op,
                                       std::span<const uint8_t> pixel_data,
                                       uint32_t tile_width,
                                       uint32_t tile_height,
                                       std::mutex &accumulator_mutex);

  aifocore::Status WriteMultiChannelTileSimple(
  ...
  std::mutex internal_mutex_; // Mutex for non-parallel WriteTile calls
