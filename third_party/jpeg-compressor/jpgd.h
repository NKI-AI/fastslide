...
class jpeg_decoder {
 public:
  enum {
    ...
    cFlagDisableSIMD = 2,
    cFlagNoYCbCrConversion = 4  // Treat YCbCr as RGB (for TIFF/SVS files)
  };
 ...
 private:
  ...
  uint8* m_huff_num
      [JPGD_MAX_HUFF_TABLES];  // pointer to number of Huffman codes per bit size
  uint8* m_huff_val
      [JPGD_MAX_HUFF_TABLES];  // pointer to Huffman codes per bit size
  jpgd_quant_t*
      m_quant[JPGD_MAX_QUANT_TABLES];  // pointer to quantization tables
  int m_scan_type;  // Gray, Yh1v1, Yh1v2, Yh2v1, Yh2v2 (CMYK111, CMYK4114 no longer supported)
  int m_comps_in_frame;  // # of components in frame
  int m_comp_h_samp
      [JPGD_MAX_COMPONENTS];  // component's horizontal sampling factor
  int m_comp_v_samp
      [JPGD_MAX_COMPONENTS];  // component's vertical sampling factor
  int m_comp_quant
      [JPGD_MAX_COMPONENTS];  // component's quantization table selector
  ...
  int m_comp_dc_tab
      [JPGD_MAX_COMPONENTS];  // component's DC Huffman coding table selector
  int m_comp_ac_tab
      [JPGD_MAX_COMPONENTS];  // component's AC Huffman coding table selector
  ...
  void H2V2Convert();
  void H2V2ConvertRGB();  // No YCbCr conversion
  uint32_t H2V2ConvertFiltered();
  void H2V1Convert();
  void H2V1ConvertRGB();  // No YCbCr conversion
  void H2V1ConvertFiltered();
  void H1V2Convert();
  void H1V2ConvertFiltered();
  void H1V1Convert();
  void H1V1ConvertRGB();  // No YCbCr conversion - for TIFF/SVS RGB data
  void gray_convert();
  void find_eoi();
