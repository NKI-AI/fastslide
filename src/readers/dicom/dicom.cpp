...
aifocore::Status DicomReader::AddFile(std::shared_ptr<DicomFile> file) {
  ...
  if (is_level) {
    DicomLevel level;
    level.file = file;
    level.width = file->total_pixel_matrix_columns;
    level.height = file->total_pixel_matrix_rows;
    level.tile_w = file->columns;
    level.tile_h = file->rows;

    ExtractPixelSpacing(metadata, level);
    ExtractObjectiveLensPower(metadata, level);

    for (const auto& existing : levels_) {
      if (existing.width == level.width && existing.height == level.height) {
        if (!file->sop_instance_uid.empty() &&
          ...
          return AIFOCORE_MAKE_STATUS(
              aifocore::StatusCode::kInvalidArgument,
              aifocore::fmt::format(
                  "Conflicting SOP Instance UIDs for {}x{} level: {} vs {}",
                  level.width, level.height, file->sop_instance_uid,
                  existing.file->sop_instance_uid));
        }
        return aifocore::Status::OkStatus();
      }
    }
      ...
      if (existing.name == assoc.name) {
        if (!file->sop_instance_uid.empty() &&
          ...
          return AIFOCORE_MAKE_STATUS(
              aifocore::StatusCode::kInvalidArgument,
              aifocore::fmt::format(
                  "Conflicting SOP Instance UIDs for '{}': {} vs {}",
                  assoc.name, file->sop_instance_uid,
                  existing.file->sop_instance_uid));
        }
        return aifocore::Status::OkStatus();
      }
...
aifocore::Status DicomReader::Initialize() {
  ...
  // Scan directory for sibling .dcm files.
...
aifocore::Result<Image> DicomReader::ReadAssociatedImage(
  ...
  AIFOCORE_ASSIGN_OR_RETURN(auto rgb, dicom::internal::DecodeDicomFrameBytes(
                                          std::span<const uint8_t>(raw, length),
                                          file->transfer_syntax, w, h));
...
Metadata DicomReader::GetMetadata() const {
  ...
  if (!levels_.empty()) {
    ...
    if (l0.file) {
      meta["dicom.series_instance_uid"] = l0.file->slide_id;
    }
  }
  return meta;
}
...
aifocore::Result<core::TilePlan> DicomReader::PrepareRequest(
  ...
  return DicomPlanBuilder::BuildPlan(request, *this);
}

aifocore::Status DicomReader::ExecutePlan(const core::TilePlan& plan,
                                          runtime::Canvas& writer) const {
  return DicomTileExecutor::ExecutePlan(plan, *this, writer);
}
