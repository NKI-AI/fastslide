...
aifocore::Result<core::TilePlan> PhilipsTiffPlanBuilder::BuildPlan(
  ...
  if (pyramid.empty()) {
    return aifocore::Status(aifocore::StatusCode::kInvalidArgument,
                            "PhilipsTIFF has no pyramid levels");
  }
  ...
  plan.operations = readers::simpletiff_plan::BuildTileReadOps(
      ...
      [&](size_t,
          const readers::simpletiff_plan::IntersectingTile& intersecting_tile) {
        return std::make_pair(
            static_cast<uint32_t>(page),
            TileCoordinate{intersecting_tile.tile_x, intersecting_tile.tile_y});
      });
