...
// CZI structures use little-endian 64-bit integers. fastslide's shared binary
// utils currently expose only 32-bit helpers, so we provide a small local
// reader.
uint16_t ChooseU16Denom(uint16_t max_val) {
  if (max_val == 0) {
    return 255;
  }
  // Find smallest bit-width that fits max_val.
  unsigned bits = 0;
...
aifocore::Result<ImageDimensions> CziReader::GetAssociatedImageDimensions(
  ...
  const int64_t embedded_base =
      found->file_pos + static_cast<int64_t>(kCziAttachmentHdrLen);
  AIFOCORE_ASSIGN_OR_RETURN(const auto sb,
                            ParseEmbeddedSingleSubblock(file, embedded_base));
...
aifocore::Result<Image> CziReader::ReadAssociatedImage(
  ...
  AIFOCORE_ASSIGN_OR_RETURN(const auto sb,
                            ParseEmbeddedSingleSubblock(file, embedded_base));
  AIFOCORE_ASSIGN_OR_RETURN(auto rgb,
                            ReadEmbeddedSubblockRgb8(file, embedded_base, sb));
